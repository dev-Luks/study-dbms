...
--WHERE WEIGHT BETWEEN 70 AND 80;  이상,이하만 됨
...
SELECT *FROM PLAYER WHERE (TEAM_ID ='K02' OR TEAM_ID ='K07') AND "POSITION" ='MF';
				--  WHERE TEAM_ID IN('K02','K07') AND "POSTITION" = 'MF';

/*PLAYER 테이블에서 TEAM_ID가 'K01'	인 선수 이름을 내 이름으로 바꾸기*/
SELET PLAYER_NAME FROM PLAYER
WHERE TEAM_ID ='K01';
...
SELECT *FROM PLAYER_NAME, NVL(NICKNAME, '없음') FROM PLAYER
...
/*PLAYER 테이블에서 HEIGHT로 총 선수명수검색*/
SELECT COUNT (NVL(HEIGHT, 0)) "총 선수수" FROM PLAYER;
...
/*EMPLOYEES 테이블에서 JOB_ID별 평균 SALARY가 10000미만인 
 * JOB_ID검색, JOB_ID 알파벳순 정렬*/
...
SELECT "POSITION" ,AVG(HEIGHT) AS "포지션별 평균 키", (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균" FROM PLAYER
WHERE HEIGHT IS NOT NULL
GROUP BY "POSITION";
